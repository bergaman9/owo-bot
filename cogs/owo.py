...
class OwO(commands.Cog):
    ...
    async def on_message(self, message):
        if message.author.id == 408785106942164992:
            if "complete your captcha to verify that you are human!" in message.content.lower():
                self.active = False
    ...
    async def owo_loop(self):
        if not self.active:  # Eğer self.active False ise, loop'u durdur
            return
        ...
        random_seconds = randint(15, 25)  # Örneğin, 20 ile 40 saniye arasında rastgele bir zaman seç
